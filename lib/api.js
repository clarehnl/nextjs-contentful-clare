...
const HEROBANNER_GRAPHQL_FIELDS = `
sys {
  id
}
slug
...
const TEXTBLOCK_GRAPHQL_FIELDS = `
  sys {
    id
  }
  slug
...
function extractHeroBannerEntries(fetchResponse) {
  console.log("fetched: ", fetchResponse?.data?.genericHeroBannerCollection?.items)
...
export async function getTextBlock(
  ...
  console.log("fetched textBlock: ", textBlock.data)
